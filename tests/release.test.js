...
test("release copy documents current scope without em dashes", () => {
  ...
  assert.equal(manifest.name, "Jeffrey Video Digest");
  assert.equal(packageJson.name, "jeffrey-video-digest");
  assert.match(read("scripts/package-extension.sh"), /jeffrey-video-digest-v\$version\.zip/);
  assert.doesNotMatch(
    [readme, chineseReadme, read("PRIVACY.md"), read("SECURITY.md")].join("\n"),
    /\bYT Digest\b/,
  );
  assert.match(readme, /^# Jeffrey Video Digest$/m);
  assert.match(
    readme,
  ...
  assert.match(
    readme,
    /permanent folder I choose[\s\S]*tell me its exact full path[\s\S]*If I need a suggestion during this first installation[\s\S]*`~\/Documents\/jeffrey-video-digest`[\s\S]*`%USERPROFILE%\\Documents\\jeffrey-video-digest`[\s\S]*do not assume either path/,
  );
  assert.match(
    readme,
  ...
  assert.match(chineseReadme, /^# Jeffrey Video Digest$/m);
  assert.match(chineseReadme, /把每个 YouTube 视频变成一份可以深入学习的资料/);
  assert.match(chineseReadme, /^## 让你的编程 Agent 帮你安装$/m);
  assert.match(
    chineseReadme,
    /我选择的长期保留文件夹[\s\S]*告诉我准确的完整路径[\s\S]*第一次安装时需要位置建议[\s\S]*`~\/Documents\/jeffrey-video-digest`[\s\S]*`%USERPROFILE%\\Documents\\jeffrey-video-digest`[\s\S]*不要假设我一定使用这些路径/,
  );
  assert.match(
    chineseReadme,
  ...
  assert.match(
    optionsPage,
    /class="customization-steps"[\s\S]*Open the extracted Jeffrey Video Digest project folder in your coding[\s\S]*Replace \[PROVIDER\] and \[MODEL\][\s\S]*Never include API keys[\s\S]*<\/ol>/,
  );
  assert.match(
    optionsPage,
    /class="prompt-reminder"[\s\S]*Before copying, replace \[PROVIDER\] and \[MODEL\]/,
  );
  assert.doesNotMatch(optionsPage, /~\/Documents\/jeffrey-video-digest/);
  assert.doesNotMatch(optionsPage, /%USERPROFILE%\\Documents\\jeffrey-video-digest/);
  ...
  assert.match(
    readme,
    /first open the exact Jeffrey Video Digest project folder that Chrome loaded through \*\*Load unpacked\*\* in your coding agent/,
  );
  assert.match(
    chineseReadme,
    /先在编程 Agent 中打开 Chrome 通过“加载已解压的扩展程序”使用的那个准确的 Jeffrey Video Digest 项目文件夹/,
  );
