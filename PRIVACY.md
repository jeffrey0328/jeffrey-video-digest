...
Jeffrey Video Digest is a GitHub-only, bring-your-own-key Chrome extension. It has no Jeffrey Video Digest account, developer-operated backend, analytics, advertising, or telemetry.

## Data the extension handles

Depending on the feature you use, Jeffrey Video Digest handles:
...
Jeffrey Video Digest sends the canonical YouTube video URL to `https://api.supadata.ai` with your Supadata API key. Supadata returns the transcript and timestamps. A Supadata key is required for transcript retrieval.
...
Requests go directly from the extension to Supadata or DeepSeek. They are authenticated with the keys you supply. Jeffrey Video Digest's developer does not proxy or receive these requests.
...
Jeffrey Video Digest uses Chrome's local extension storage, not a Jeffrey Video Digest cloud service.
...
- delete individual saved notes in Jeffrey Video Digest;
...
Jeffrey Video Digest uses Chrome permissions for these purposes:

- `sidePanel`: display the Jeffrey Video Digest interface beside YouTube.
...
Jeffrey Video Digest does not use these permissions to monitor general browsing activity.

## No sale or advertising use

Jeffrey Video Digest does not sell personal information, build advertising profiles, or share data with data brokers. It does not include analytics SDKs.
