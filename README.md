...
Turn every YouTube video into a resource for deep learning. Jeffrey Video Digest brings transcripts, bilingual translation, AI overviews, explanations, and timestamped notes into one Chrome side panel, so you can study ideas and language without losing your place.
...
Jeffrey Video Digest is a bring-your-own-key project installed locally from GitHub. It is not available through the Chrome Web Store, does not include API credits, and does not run a developer-operated server.

## Install with your coding agent

You do not need to understand the code or use the command line. Send this message to your coding agent:

> Download or clone this project into a permanent folder I choose, tell me its exact full path, and use that same folder for Chrome's Load unpacked step. If I need a suggestion during this first installation, offer `~/Documents/jeffrey-video-digest` on macOS or Linux, or `%USERPROFILE%\Documents\jeffrey-video-digest` on Windows, but do not assume either path. Walk me through installation and setup in simple terms. https://github.com/jeffrey0328/jeffrey-video-digest

Your agent should:

1. Ask where you want to keep the project, download or clone it there, and tell you the exact full path. If you want a suggestion, it can offer `~/Documents/jeffrey-video-digest` on macOS or Linux, or `%USERPROFILE%\Documents\jeffrey-video-digest` on Windows.
...
Never paste an API key into an AI chat, source file, screenshot, or public message. Enter keys yourself, directly in the Jeffrey Video Digest Settings page. Your coding agent can point to the correct field without seeing the key.

## Install manually

If you prefer to do it yourself:

1. Open [github.com/jeffrey0328/jeffrey-video-digest](https://github.com/jeffrey0328/jeffrey-video-digest).
2. Choose **Code**, then **Download ZIP**.
3. Choose a permanent folder and unzip the project there. Optional suggestions are `~/Documents/jeffrey-video-digest` on macOS or Linux, or `%USERPROFILE%\Documents\jeffrey-video-digest` on Windows. You may use a different folder.
4. In Chrome, open `chrome://extensions`.
5. Turn on **Developer mode**.
6. Click **Load unpacked**.
7. Select the exact project folder you chose, which must contain `manifest.json`.
8. Pin Jeffrey Video Digest from Chrome's Extensions menu if you want quick access.

Because this is an unpacked extension, it does not update automatically. After downloading an update or changing local files, click **Reload** on the Jeffrey Video Digest card at `chrome://extensions`, then refresh open YouTube tabs. Moving or deleting the source folder breaks the unpacked extension until you load it again from the new location.

## Set up your API keys

Jeffrey Video Digest needs two keys under your own provider accounts:
...
5. Copy the key and paste it into **Supadata API key** in Jeffrey Video Digest Settings.
...
3. Choose **Create new API key**, give it a recognizable name such as `Jeffrey Video Digest`, and create it.
4. Copy the key immediately. The full key may only be shown once.
5. Paste it into **DeepSeek API key** in Jeffrey Video Digest Settings.
...
Open **Settings** from the side panel. You can also open the Jeffrey Video Digest **Options** page from its card at `chrome://extensions` or by right-clicking its toolbar icon. Paste keys only into these Settings fields. Never paste a key into an AI chat, repository file, screenshot, or public message.
...
## Use Jeffrey Video Digest

1. Open a standard YouTube watch page with captions.
2. Click the Jeffrey Video Digest extension icon to open the side panel.
...
- Native subtitle tracks returned by Supadata. Jeffrey Video Digest prefers English when available, but may show another native language.
...
Jeffrey Video Digest forces Supadata's `mode=native`. It does not request AI-generated transcripts or perform local audio transcription when native captions are unavailable.
...
- A generated transcript costs **2 credits per video minute**. Jeffrey Video Digest does not use this path because it forces `mode=native`.
...
DeepSeek usage is separate from Supadata. DeepSeek may apply its own free quota, rate limits, or charges. Jeffrey Video Digest does not collect payments or resell access. Set spending limits and monitor both accounts. The estimate below explains the current DeepSeek translation cost.
...
Jeffrey Video Digest uses plain HTML, CSS, and JavaScript with no build step, so it is a friendly starting point for agent-assisted projects. Ideas to try:
...
If you want another AI provider or model, first open the exact Jeffrey Video Digest project folder that Chrome loaded through **Load unpacked** in your coding agent. Then open Jeffrey Video Digest Settings and use **Copy customization prompt**. Replace the `[PROVIDER]` and `[MODEL]` placeholders before sending it. Do not include any API key in the prompt or chat. After the agent updates your local copy, enter the key yourself in the Settings field it identifies.

## Privacy and data flow

Jeffrey Video Digest makes provider requests directly from the extension:
...
There is no Jeffrey Video Digest account system, advertising, analytics, or telemetry. Supadata and DeepSeek still receive data under their own terms and privacy policies. See [PRIVACY.md](PRIVACY.md) for details.

## Troubleshooting

### The Digest button is missing on a YouTube video

- At `chrome://extensions`, find Jeffrey Video Digest and click **Reload**, then refresh the YouTube tab.
...
- At `chrome://extensions`, confirm Jeffrey Video Digest is enabled and click **Reload**.
...
### Jeffrey Video Digest asks for setup
...
Jeffrey Video Digest will not fall back to generated transcription.
